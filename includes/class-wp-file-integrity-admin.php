...
if (!class_exists('WP_File_Integrity_Admin')) {
    ...
    class WP_File_Integrity_Admin {
        ...
        public function __construct($core) {
            ...
            // Add AJAX action for retrieving results
            add_action('wp_ajax_wp_file_integrity_get_results', array($this, 'ajax_get_scan_results'));
        }
        
        /**
         * AJAX handler to get scan results
         */
        public function ajax_get_scan_results() {
            check_ajax_referer('wp_file_integrity_get_results_nonce', 'nonce');
                                    ...
                                    success: function(response) {
                                        if (response.success) {
                                            ...
                                            if (data.percentage >= 100) {
                                                ...
                                                // Replace progress section with scan results via AJAX
                                                $.ajax({
                                                    url: ajaxurl,
                                                    type: 'GET',
                                                    data: {
                                                        action: 'wp_file_integrity_get_results',
                                                        nonce: '<?php echo wp_create_nonce('wp_file_integrity_get_results_nonce'); ?>'
                                                    },
                                                    success: function(response) {
                                                        if (response.success) {
                                                            // Remove the progress container
                                                            $('.wp-file-integrity-progress').remove();
                                                            
                                                            // Append the results HTML after the form
                                                            $('form[name="wp_file_integrity_run_check"]').after(response.data.html);
                                                        }
                                                    }
                                                });
                                            }
